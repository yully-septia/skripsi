...
<div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800" >
    <?php foreach ($data_kda as $data):?>
    <form action="<?= base_url('data/update/'.$data['id_data_kec']); ?>" method="post">
        <?= csrf_field(); ?>
        ...
        <select id="kategori" name="kategori" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
            <option value="<?= $data['id_kategori']; ?>" <?= (old('kategori') == $data['kategori'] ? 'selected' : '') ?> class="text-slate-500 text-sm"><?= $data['kategori']; ?></option>
        ...
        <option value="<?= $data['id_indikator']; ?>" <?= (old('indikator') == $data['indikator'] ? 'selected' : '') ?> class="text-slate-500 text-sm"><?= $data['indikator']; ?></option>
            <?php  ?>
        </select>
        </label>
        ...
        <option value="<?= $data['id_var']; ?>" <?= (old('variabel') == $data['variabel'] ? 'selected' : '') ?> class="text-slate-500 text-sm"><?= $data['variabel']; ?></option>
            <?php  ?>
        </select>
        </label>
        ...
        <option value="<?= $data['id_tahun']; ?>" <?= (old('tahun') == $data['tahun'] ? 'selected' : '') ?> class="text-slate-500 text-sm"><?= $data['tahun']; ?></option>
        ...
        <option value="<?= $data['id_kec']; ?>" <?= (old('kecamatan') == $data['kecamatan'] ? 'selected' : '') ?> class="text-slate-500 text-sm"><?= $data['kecamatan']; ?></option>
        ...
        <div class="block mt-4 text-sm">
            <button type="submit" value="Update" class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                Simpan
            </button>
        </div>
    </form>
